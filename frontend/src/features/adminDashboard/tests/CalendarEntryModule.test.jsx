...
describe('CalendarEntry Module - Admin Dashboard', () => {
    afterEach(() => {
        cleanupAuth();
    });
    ...
    const createCalendarEntry = async (title, date, start_time = '09:00', end_time = '10:00') => {
        const response = await apiClient.post('/calendar-entries', { title, date, start_time, end_time, user_id: '1' });
        return response;
    };
    ...
    test('allows admin to create a new Calendar Entry', async () => {
        const uniqueTitle = `Client Meeting ${Date.now()}`;
    ...
    test('allows admin to edit an existing Calendar Entry', async () => {
        ...
        const originalTitle = `Edit Sprint Planning ${Date.now()}`;
        ...
        const editButtons = screen.getAllByText(/edit/i);
        // Assuming the last edit button corresponds to the latest entry
    ...
    test('allows admin to delete a Calendar Entry', async () => {
        ...
        const calendarEntryName = `Delete Sprint Planning ${Date.now()}`;
        ...
        const deleteButtons = screen.getAllByText(/delete/i);
        // Assuming the last delete button corresponds to the latest entry
